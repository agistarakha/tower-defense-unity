...
public class LevelManager : MonoBehaviour
{
    ...
    {
        GameObject newBulletObj = _spawnedBullets.Find(bullet => !bullet.gameObject.activeSelf && bullet.name.Contains(prefab.name))?.gameObject;
    ...
    public void ExplodeAt(Vector2 point, float raidus, int damage)
    {
        foreach (Enemy enemy in _spawnedEnemies)
        {
            if (enemy.gameObject.activeSelf)
            {
                if (Vector2.Distance(enemy.transform.position, point) <= raidus)
                {
